...
description: Use when setting up a Laravel worktree for local development with Laravel Valet, or when user asks to work on a feature branch in isolation
license: MIT
---

# Laravel Valet Worktree Setup

## Overview

Sets up a git worktree for Laravel projects served by Laravel Valet, ensuring the worktree has its own site URL and properly configured environment.

**Announce at start:** "I'm using the laravel-valet-worktree skill to set up an isolated Laravel workspace with Valet."
...
- User mentions "worktree" and the project uses Laravel Valet
...
- **Laravel Valet** installed and running on macOS
...
**First, detect the project name** (used for the Valet site URL to avoid conflicts between projects with the same branch name):
...
```
AskUserQuestion:
  question: "Project name detected as '$PROJECT_NAME'. This will be used for the Valet URL (e.g., projectname-branchname.test). Is this correct?"
  header: "Project"
  options:
...
**Note:** Both project name and branch name will be sanitized for the Valet link (slashes replaced with dashes, spaces removed). For example, project `my-app` with branch `feature/new-feature` becomes `my-app-feature-new-feature.test`.
...
This creates a site at `https://$SITE_NAME.test` (e.g., `https://myproject-feature-branch.test`)
...
# Update APP_URL to match Valet site (use HTTP, not HTTPS)
sed -i '' "s|APP_URL=.*|APP_URL=https://$SITE_NAME.test|" /path/to/worktree/.env
...
echo "DEBUGBAR_ENABLED=false" >> /path/to/worktree/.env
echo "TELESCOPE_ENABLED=false" >> /path/to/worktree/.env
echo "PULSE_ENABLED=false" >> /path/to/worktree/.env
```
...
valet composer install --no-interaction  # Append user's custom flags here if provided
...
### 5. Kill Existing Vite Processes & Start Dev Server
...
**After setup completes, tell the user:** "When you're ready to finish your work, run `/laravel-valet-worktree` again and I'll help you integrate or clean up your changes."
...
- `$SITE_NAME` - Combined name for Valet: `$PROJECT_NAME-$SANITIZED_BRANCH_NAME` (e.g., `my-laravel-app-feature-login`)

| Step              | Command                                                            |
|-------------------|--------------------------------------------------------------------|
| Create worktree   | `git worktree add .worktrees/$SITE_NAME -b $BRANCH_NAME`           |
| Link to Valet     | `valet link $SITE_NAME`                                            |
| Secure Valet      | `valet secure $SITE_NAME`                                          |
| Copy .env         | `cp ../../../.env .env`                                            |
| Update APP_URL    | `sed -i '' "s\|APP_URL=.*\|APP_URL=http://$SITE_NAME.test\|" .env` |
| Install PHP deps  | `composer install --no-interaction`                                |
| Install Node deps | `npm install`                                                      |
| Clear cache       | `valet php artisan config:clear && valet php artisan cache:clear`  |
| Kill old Vite     | `pkill -f "node.*vite"`                                            |
| Start dev         | `npm run dev`                                                      |
| Unlink            | `valet unlink $SITE_NAME`                                          |
| Unsecure          | `valet unsecure $SITE_NAME`                                        |
...
- **Fix:**
    ...
    2. Run `valet php artisan config:clear`
...
- **Fix:**
    1. Secure the site: `valet secure $SITE_NAME`
...
- **Fix:** Update APP_URL to match Valet site name

### Missing vendor directory
- **Cause:** Worktrees don't share vendor/
- **Fix:** Run `valet composer install` in worktree
...
### Permission issues with Valet
- **Cause:** Valet needs to access worktree
- **Fix:** Ensure worktree is in a Valet-accessible path
...
- [ ] Valet link created (`valet link $SITE_NAME`)
- [ ] Site is NOT secured (use HTTP, not HTTPS)
- [ ] .env copied and APP_URL updated to `https://$SITE_NAME.test`
- [ ] DEBUGBAR_ENABLED=false added
- [ ] TELESCOPE_ENABLED=false added
- [ ] `composer install` completed successfully
- [ ] `npm install` completed successfully
- [ ] Old Vite processes killed
- [ ] `npm run dev` running from worktree directory
- [ ] Site accessible at `https://$SITE_NAME.test` (no white page)
